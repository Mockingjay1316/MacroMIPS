...
int len,len_,ln,l;								//len: length of input  ln: number of name
char com[12345];							//input
int nx,ny,ang,col,is_down;					//nx ny: now x y  ang: angle  is_down: about pen
char name[123][12];						//name<=>id
int func_place[123];						//place of functions
int variable_value[123][12],topv[123];
int tmp_variable[123][123],lv[123];
int return_stack[123],topr;
int oper_stack[123],topo,num_stack[123],topn;
int Mx,My;
//uint8_t Map[805][605];
unsigned int vga_start = 0x82000000;
...
void op()
{	//printf("\e[1;1H\e[2J");                        //锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷一锟斤拷
printf("in op Mx:%d My:%d\n",Mx,My);
    int i,j,k,l,flag=0;
    for(j=My-1;j>=0;j-=10){
		for(i=0;i<Mx;i+=10)
		{	int tmp=255;
...
printf("in op final flag is:%d\n",flag);
}
void readAll() {
    int ret, i = 0, j = 0, x = 0;						//锟斤拷锟斤拷x锟斤拷锟节硷拷录锟斤拷锟轿伙拷诘锟斤拷锟斤拷锟絰锟斤拷位锟斤拷 
    int flag = 0;                                       //0:init  1:get 27  2:get 91
    const char deleteKey = 127;
    const char dir_27 = 27;                             //锟斤拷锟诫方锟斤拷锟斤拷锟斤拷锟斤拷蔚玫锟?7/91/65~68
...
//printf("in readAll:when get #, i:%d\n",i);
			i++;
//printf("in readAll:when get #, i:%d len:%d len_:%d\n",i,len,len_);
            len = i;
len_ = i;
//len__=i;
//printf("in readAll:when get #, len:%d, len_:%d\n",len,len_);
            break;
        }
            ...
            printf("%c[%d;%dH",27,1,1);                    //锟窖癸拷锟斤拷锟斤拷锟斤拷锟絠锟叫碉拷j锟斤拷
		    for(j=0;j<i;j++)
                if(com[j]=='\n')printf("\n");
                else printf(" ");
            ...
            if(x==0) {
				com[i++]=c;
				printf("%c",com[i-1]);
//printf("in readAll:when getchar, i:%d len:%d\n",i,len);
			} else {
...
{	
	*(int*)(vga_start + (599-y)*800+x) = z;
//printf("%d %d %d\n",x,y,z);
	//Map[x][y]=(uint8_t)(z&255);
}
void init()
...
//printf("in init Mx:%d My:%d map400400:%d\n",Mx,My,Map[400][400]);
	//memset(Map,255,sizeof(Map));
//printf("in init 2# Mx:%d My:%d map400400:%d\n",Mx,My,Map[400][400]);
//int flag=0;printf("bef flag:%d\n",flag);
//for(i=0;i<Mx;i++){for(j=0;j<My;j++)if(Map[i][j]==0)flag++;if((i&15)==0)printf("%d\n",i);}
//printf("in init aft flag:%d\n",flag);
    printf("\e[1;1H\e[2J");                        //锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷一锟斤拷
    readAll();
//printf("aft readAll,in init : len:%d\n",len);
	com[--len]='\0';
//printf("aft readAll,in init : len:%d\n",len);
//printf("**************************************\n%s\n************************************************************&\n",com);
//printf("%c[%d;%dH",27,1,1);
	*(int*)(0x82075300) = 2;    // 鏇存敼鏄剧ず妯″紡涓篖OGO
    sleep(2000);
	int i,j;for(i=0;i<Mx;i++)for(j=0;j<My;j++)paint(i,j,255);//Map[i][j]=255;
}
void draw_line(int x0,int y0,int x1,int y1,int z)
{	int i,x,y,dx,dy,e;
//printf("in draw_line %d %d %d %d\n",x0,y0,x1,y1);
	...
	{	dx=abs(x1-x0),dy=abs(y1-y0),e=-dx;
		...
		for(i=0;i<=dx;i++)
		{	paint(x,y,z);
//printf("in draw_line: x:%d y:%d z:%d\n",x,y,(uint8_t)(z&255));
			x+=x1>x0?1:-1,e+=dy+dy;
			if(e>=0)y+=y1>y0?1:-1,e-=dx+dx;
		}
	}
	else
...
//printf("getnumber: res=%d\n",res);
		return res<<16;
	}
}
int getres(int a,int x,int b)
{	if(x==8)return mult((a>>8),(b>>8));
	...
	if(x==27){printf("in getres check>: %d > %d ? %d  %d %d\n",a,b,a>b?1:0,a>>16,b>>16);return a>b?1:0;}
...
//printf("getcomm: fd %d\n",x);
			tmpx=nx,tmpy=ny;
            if(x>=(1<<23))
            {   nx+=mult((x>>16),Cos[ang]);
			    ny+=mult((x>>16),Sin[ang]);
            }
			else
            {   nx+=mult((x>>8),Cos[ang])>>8;
			    ny+=mult((x>>8),Sin[ang])>>8;
//printf("in getcomm fd  x:%d  ang:%d cos:%d x*cos:%d\n",x,ang,Cos[ang],(x>>8)*Cos[ang]>>8);
//printf("in getcomm fd  x:%d  ang:%d sin:%d x*sin:%d\n****************8\n",x,ang,Sin[ang],(x>>8)*Cos[ang]>>8);
            }
...
//printf("getcomm: lt %d\n",x);
		...
		{	l+=2;
			...
			{	l+=4;
//printf("in getcomm if boolean:%d\n",x);
				if(x!=0)
				{	
printf("in getcomm if true\n");
				...
				{	
printf("in getcomm if false\n");
					while((getname()!=4||com[l]!='e'||com[l+1]!='l'||com[l+2]!='s'||com[l+3]!='e')&&
						  (getname()!=5||com[l]!='e'||com[l+1]!='n'||com[l+2]!='d'||com[l+3]!='i'||com[l+4]!='f'))
						l+=getname();
printf("after if skip l:%d %c%c%c%c%c",l,com[l-4],com[l-3],com[l-2],com[l-1],com[l]);
					if(getname()==4&&com[l]=='e'&&com[l+1]=='l'&&com[l+2]=='s'&&com[l+3]=='e')
					{	l+=4;
						while(getname()!=5||com[l]!='e'||com[l+1]!='n'||com[l+2]!='d'||com[l+3]!='i'||com[l+4]!='f')
							getcomm();
					...
					l+=5;
printf("after if end l:%d %c%c%c%c%c",l,com[l-4],com[l-3],com[l-2],com[l-1],com[l]);
				}
			}
			return;
		}
...
//printf("#600+ in work l:%d\n",l);
}
int main(int argc, char **argv)
{   init();
//printf("main l:%d len:%d\n",l,len);
	for(skip_space();l<len;)/*printf("main l:%d len: %d",l,len),*/work(),skip_space();
	//op();
	sleep(10000);
    *(int*)(0x82075300) = 0;    // 鏇存敼鏄剧ず妯″紡涓篊MD
	return 0;
}
